...
out_csv_full     <- file.path(output_dir, "GO_Enrichment_Defense_Oomycete_07072025.csv")
out_csv_filtered <- file.path(output_dir, "GO_Enrichment_Defense_Oomycete_Filtered_GT3clusters_07072025.csv")
out_rds_filtered <- file.path(output_dir, "GO_Enrichment_Defense_Oomycete_Filtered_GT3clusters_07072025.rds")
out_pdf          <- file.path(output_dir, "GO_Enrichment_Defense_Oomycete_Filtered_GT3clusters_07072025.pdf")
...
# ---------- Focus on defense/immune/oomycete terms ----------
defense_terms <- go_all %>%
  filter(str_detect(
    Description,
    regex("defense|immune|oomycete|oxidative|pathogen|jasmonic|salicylic|fungus|hypersensitive",
          ignore_case = TRUE)
  )) %>%
...
if (nrow(defense_terms) == 0) stop("No defense-related GO terms found.")

# ---------- Filter GO terms found in > 3 clusters ----------
term_cluster_freq <- defense_terms %>%
  distinct(Description, cluster) %>%
  count(Description, name = "n_clusters")

defense_terms_filtered <- defense_terms %>%
  left_join(term_cluster_freq, by = "Description") %>%
  filter(n_clusters > 3)

if (nrow(defense_terms_filtered) == 0) {
  stop("After filtering GO terms present in <= 3 clusters, no defense terms remain.")
}

# ---------- Add labels before saving RDS ----------
defense_terms_filtered$cluster <- as.character(defense_terms_filtered$cluster)
defense_terms_filtered$cluster_label <- cluster_labels[defense_terms_filtered$cluster]
defense_terms_filtered$cluster_label[is.na(defense_terms_filtered$cluster_label)] <- defense_terms_filtered$cluster
defense_terms_filtered$cluster_label <- factor(
  defense_terms_filtered$cluster_label,
  levels = all_cluster_levels
)

write_csv(defense_terms_filtered, out_csv_filtered)
saveRDS(defense_terms_filtered, out_rds_filtered)

cat("Filtered defense CSV saved to:", out_csv_filtered, "\n")
cat("Filtered defense RDS saved to:", out_rds_filtered, "\n")

# ---------- Check remaining terms per cluster ----------
cluster_summary <- defense_terms_filtered %>%
  distinct(cluster, Description) %>%
  count(cluster, name = "n_terms_remaining")

cat("\nDefense GO terms remaining per cluster after filtering:\n")
print(cluster_summary)

missing_clusters <- setdiff(all_cluster_ids, unique(as.character(defense_terms_filtered$cluster)))
if (length(missing_clusters) > 0) {
  cat("\nClusters with zero remaining defense GO terms after filtering:\n")
...
p <- ggplot(
  ...
  labs(
    title = "GO Enrichment: Defense & Oomycete Response (terms in >3 clusters)",
    x = "Cluster",
